...
if (cancelAnimationFrameFn == null || requestAnimationFrameFn == null) {
  ...
  requestFrame = function requestAnimationFrameViaSetTimeout(callback: () => void) {
  ...
  cancelFrame = function cancelFrameFn([animationFrameID, timeoutID]: [any, any]) {
  ...
  requestFrame = function requestAnimationFrameWithSetTimeoutFallback(callback: () => void) {
    ...
    timeoutID = setTimeoutFn(function timeoutCallback() {
      cancelAnimationFrameFn(animationFrameID);
      callback();
    }, TIMEOUT_DURATION);
...
export default function createDetectElementResize(nonce: any) {
  ...
  if (!attachEvent) {
    ...
    scrollListener = function (this: any, e: any) {
      ...
      this.__resizeRAF__ = requestFrame(function animationFrame() {
        if (checkTriggers(element)) {
          ...
          element.__resizeListeners__.forEach(function forEachResizeListener(fn: any) {
            fn.call(element, e);
          });
        }
      });
  ...
  const createStyles = function (doc: any) {
    if (!doc.getElementById('detectElementResize')) {
      ...
      const css =
        `${animationKeyframes || ''}.resize-triggers { ${animationStyle || ''}visibility: hidden; opacity: 0; } ` +
    ...
    if (attachEvent) {
      ...
      if (!element.__resizeTriggers__) {
        ...
        if (animationStartEvent) {
          element.__resizeTriggers__.__animationListener__ = function animationListener(e: any) {
