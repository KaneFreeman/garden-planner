...
const PastSlotPlants = ({ slot }: PastSlotPlantsProps) => {
  ...
  const plantClick = useCallback((instance: PlantInstance) => {
    console.log('plant click', instance);
  }, []);
  ...
  const renderPlantSlot = useCallback((key: string, instance: PlantInstance, index: number) => {
    ...
    return <SlotListItem key={`${key}-${index}`} instance={instance} onClick={plantClick} primary={plant.name} showStatus={false} />;
  }, [plantClick, plantsById]);

  return plantInstances.length > 0 ? (
    <Box sx={{ width: '100%' }}>
      <Typography
        variant="subtitle1"
        component="div"
